...
interp (ExprC expr t1 t2 l)
       | (isValue expr) && t1 /= Dyn && t2 /= Dyn = expr
       | (isValue expr) == False && t1 /= Dyn && t2 /= Dyn = (ExprC (interp expr) t1 t2 l)
       | (isValue expr) && t1 /= Dyn && t2 == Dyn = ground (ExprC expr t1 Dyn l)
       | (isValue expr) && t1 == Dyn && t2 /= Dyn = expand (ExprC expr Dyn t2 l)
